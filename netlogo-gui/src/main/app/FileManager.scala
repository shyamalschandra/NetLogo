...
import org.nlogo.swing.{ FileDialog, ModalProgressTask, OptionDialog, UserAction }
...
object FileManager {
  ...
  //TODO: This only applies on windows, it's handled separately on Mac
  ...
  extends ExceptionCatchingAction(I18N.gui.get("menu.file.import.hubNetClientInterface") + Ellipsis, parent) {
    putValue(UserAction.ActionCategoryKey,    UserAction.FileCategory)
    putValue(UserAction.ActionSubcategoryKey, UserAction.FileImportSubcategory)
  ...
  class SaveAsNetLogoWebAction(manager: FileManager, workspace: AbstractWorkspaceScala, modelSaver: ModelSaver, parent: Component)
    ...
    putValue(UserAction.ActionCategoryKey, UserAction.FileCategory)
    putValue(UserAction.ActionGroupKey,    UserAction.FileShareGroup)
