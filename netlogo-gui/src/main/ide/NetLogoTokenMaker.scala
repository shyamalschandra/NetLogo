...
class NetLogoTokenMaker extends TokenMakerBase {
  ...
  def rstaTokenType(t: Token): Int = {
    ...
    t.tpe match {
      ...
      case Literal => t.value match {
        ...
        case _         => TokenTypes.DATA_TYPE
      }
      case Ident => TokenTypes.IDENTIFIER
  ...
  def getTokenList(seg: Segment, initialTokenType: Int, offset: Int): RstaToken = {
    ...
    def netlogoTokenToRstaToken(netLogoToken: Token, lastToken: Option[TokenImpl]): TokenImpl = {
      val next = new TokenImpl(seg, netLogoToken.start, netLogoToken.end - 1, offset + netLogoToken.start, rstaTokenType(netLogoToken), 0)
